<!DOCTYPE html>
<html lang="id">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Kelola Resepsionis - Admin StayEase</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@600;700&family=DM+Sans:wght@400;500;600&display=swap" rel="stylesheet">
    <script>
        tailwind.config = {
            theme: {
                extend: {
                    colors: {
                        forest: { 50: '#f0f7f0', 100: '#dceddc', 200: '#b9dbb9', 300: '#8cc28c', 400: '#5fa35f', 500: '#3d843d', 600: '#2d6a2d', 700: '#1e4d1e', 800: '#143614', 900: '#0c220c' },
                    },
                    fontFamily: { display: ['Playfair Display', 'serif'], body: ['DM Sans', 'sans-serif'] },
                }
            }
        }
    </script>
    <style>
        body { font-family: 'DM Sans', sans-serif; }
        @keyframes fadeUp { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: translateY(0); } }
        .fade-up { animation: fadeUp .45s ease both; }
    </style>
</head>

<body class="bg-[#FFF4DE] min-h-screen flex flex-col lg:flex-row">

    @include('components.sidebar_admin')

    <main class="flex-1 overflow-y-auto px-4 md:px-10 py-6 md:py-10">

        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10 fade-up">
            <div>
                <h1 class="font-display text-4xl font-bold text-forest-900">Data Resepsionis</h1>
                <p class="text-forest-500 mt-2 text-sm uppercase tracking-widest font-semibold">Manajemen Staf & Hak Akses</p>
            </div>

            <button id="openCreateModal"
                class="w-full sm:w-auto bg-forest-700 hover:bg-forest-800 text-white px-8 py-4 rounded-md shadow-lg shadow-forest-100 transition-all active:scale-95 flex items-center justify-center gap-3 font-bold text-sm">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" />
                </svg>
                TAMBAH RESEPSIONIS
            </button>
        </div>
        {{-- notif --}}
        @if (session('success'))
            <div class="mb-6 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 px-6 py-4">
                {{ session('success') }}
            </div>
        @endif

        @if ($errors->any())
            <div class="mb-6 p-4 bg-red-50 border border-red-200 text-red-700 rounded-lg text-sm font-semibold fade-up">
                <ul class="list-disc pl-5 space-y-1">
                    @foreach ($errors->all() as $error)
                        <li>{{ $error }}</li>
                    @endforeach
                </ul>
            </div>
        @endif

        <!-- SEARCH & FILTER BAR -->
        <form id="searchFilterForm" method="GET" action="{{ route('admin.resepsionis') }}" class="mb-6 flex flex-col md:flex-row gap-4 items-center justify-between fade-up" style="animation-delay: 0.05s">
            <div class="w-full md:w-96 relative">
                <span class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none text-gray-400">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                    </svg>
                </span>
                <input id="searchResepsionis" name="search" type="text" value="{{ $search ?? '' }}"
                    class="w-full rounded-xl border border-forest-100 bg-white pl-11 pr-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none transition-all shadow-sm"
                    placeholder="Cari nama resepsionis..." />
            </div>

            <div class="w-full md:w-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                <div class="flex items-center gap-3">
                    <label for="filterUrutan" class="text-sm font-semibold text-forest-800 whitespace-nowrap">Urutkan Nama:</label>
                    <select id="filterUrutan" name="order"
                        class="rounded-xl border border-forest-100 bg-white px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none transition-all shadow-sm cursor-pointer font-medium">
                        <option value="asc" {{ ($order ?? 'asc') === 'asc' ? 'selected' : '' }}>A - Z (Alfabetis)</option>
                        <option value="desc" {{ ($order ?? 'asc') === 'desc' ? 'selected' : '' }}>Z - A (Kebalikan)</option>
                    </select>
                </div>
                <button type="submit" class="w-full sm:w-auto px-5 py-3 bg-forest-700 hover:bg-forest-800 text-white text-sm font-semibold rounded-xl transition-all whitespace-nowrap">
                    Cari
                </button>
            </div>
        </form>

        <div class="bg-white rounded-md shadow-sm border border-forest-100 overflow-hidden fade-up" style="animation-delay: 0.1s">
            <div class="overflow-x-auto">
                <table class="w-full text-left whitespace-nowrap min-w-[700px]">
                    <thead>
                        <tr class="bg-forest-800 text-white text-[11px] uppercase tracking-[0.2em]">
                            <th class="px-8 py-6 font-semibold">ID Staf</th>
                            <th class="px-6 py-6 font-semibold">Nama Resepsionis</th>
                            <th class="px-6 py-6 font-semibold">Alamat Email</th>
                            <th class="px-6 py-6 font-semibold text-center">No HP</th>
                            <th class="px-8 py-6 font-semibold text-center border-l border-forest-700">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($resepsionis as $item)
                            <tr class="hover:bg-forest-50/50 transition-colors group resepsionis-row" data-name="{{ strtolower($item->name) }}">
                                <td class="px-8 py-6 text-sm font-medium text-gray-700">{{ $item->id_resepsionis }}</td>
                                <td class="px-6 py-6">
                                    <span class="font-bold text-forest-900 name-label">{{ $item->name }}</span>
                                </td>
                                <td class="px-6 py-6 text-sm text-black">{{ $item->email }}</td>
                                <td class="px-6 py-6 text-center text-sm text-black">{{ $item->no_hp ?? '-' }}</td>
                                <td class="px-8 py-6 border-l border-forest-600/40">
                                    <div class="flex justify-center gap-8">
                                        <!-- ICON EDIT (Amber) -->
                                        <button data-id="{{ $item->id }}" data-name="{{ $item->name }}" data-email="{{ $item->email }}" data-nohp="{{ $item->no_hp }}"
                                            class="btn-edit text-amber-500 hover:text-amber-700" title="Edit Data">
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                            </svg>
                                        </button>
                                        <!-- ICON DELETE (Red) -->
                                        <button data-id="{{ $item->id }}" data-name="{{ $item->name }}"
                                            class="btn-delete text-red-500 hover:text-red-700" title="Hapus Data">
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-4v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                            </svg>
                                        </button>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr id="emptyRow">
                                <td colspan="5" class="px-8 py-10 text-center text-sm text-gray-500 font-semibold">Belum ada data resepsionis.</td>
                            </tr>
                        @endforelse
                        <tr id="noResultRow" class="hidden">
                            <td colspan="5" class="px-8 py-10 text-center text-sm text-gray-500 font-semibold">Data resepsionis tidak ditemukan.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            @include('components.pagination', ['paginator' => $resepsionis])
        </div>
    </main>

    {{-- Create Modal --}}
    <div id="createModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 px-4 py-8">
        <div class="w-full max-w-lg rounded-3xl bg-white shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div class="flex items-center justify-between px-8 py-6 border-b border-gray-200 flex-shrink-0">
                <div>
                    <h2 class="text-2xl font-bold text-slate-900">Tambah Resepsionis</h2>
                    <p class="text-sm text-slate-500 mt-1">Tambahkan data resepsionis baru ke dalam sistem</p>
                </div>
                <button type="button" class="modal-close text-slate-500 hover:text-slate-900">✕</button>
            </div>
            <form action="{{ route('admin.resepsionis.store') }}" method="POST" class="flex-1 overflow-y-auto px-8 py-6 space-y-5">
                @csrf
                <div class="space-y-4">
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Nama</label>
                        <input name="name" type="text" required class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none" placeholder="Masukkan nama" />
                    </div>
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Alamat Email</label>
                        <input name="email" type="email" required class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none" placeholder="contoh@gmail.com" />
                    </div>
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Nomor HP / WhatsApp</label>
                        <input name="no_hp" type="text" required pattern="[0-9]+" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none" placeholder="Contoh: 08123456789" />
                    </div>
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Password</label>
                        <input name="password" type="password" required minlength="8" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none" placeholder="Minimal 8 karakter" />
                    </div>
                </div>
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" class="modal-close px-6 py-3 rounded-xl bg-slate-100 text-slate-700 font-bold hover:bg-slate-200 transition">Batal</button>
                    <button type="submit" class="px-6 py-3 rounded-xl bg-forest-700 text-white font-bold hover:bg-forest-800 transition">Simpan</button>
                </div>
            </form>
        </div>
    </div>

    {{-- Edit Modal --}}
    <div id="editModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 px-4 py-8">
        <div class="w-full max-w-lg rounded-3xl bg-white shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div class="flex items-center justify-between px-8 py-6 border-b border-gray-200 flex-shrink-0">
                <div>
                    <h2 class="text-2xl font-bold text-slate-900">Edit Resepsionis</h2>
                    <p class="text-sm text-slate-500 mt-1">Perbarui informasi data akun resepsionis.</p>
                </div>
                <button type="button" class="modal-close text-slate-500 hover:text-slate-900">✕</button>
            </div>
            <form id="editForm" method="POST" class="flex-1 overflow-y-auto px-8 py-6 space-y-5">
                @csrf
                @method('PUT')
                <div class="space-y-4">
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Nama</label>
                        <input id="editName" name="name" type="text" required class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none" />
                    </div>
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Alamat Email</label>
                        <input id="editEmail" name="email" type="email" required class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none" />
                    </div>
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Nomor HP / WhatsApp <span class="text-xs text-gray-400 font-normal">(Wajib)</span></label>
                        <input id="editNoHp" name="no_hp" type="text" required pattern="[0-9]+" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none" />
                    </div>
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Password Baru</label>
                        <input name="password" type="password" minlength="8" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none" placeholder="Kosongkan jika tidak ingin mengubah" />
                    </div>
                </div>
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" class="modal-close px-6 py-3 rounded-xl bg-slate-100 text-slate-700 font-bold hover:bg-slate-200 transition">Batal</button>
                    <button type="submit" class="px-6 py-3 rounded-xl bg-forest-700 text-white font-bold hover:bg-forest-800 transition">Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>

    {{-- Delete Modal --}}
    <div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 px-4 py-8">
        <div class="w-full max-w-md rounded-3xl bg-white shadow-2xl overflow-hidden">
            <div class="px-8 py-6 border-b border-gray-200 flex justify-between items-center">
                <h3 class="text-xl font-bold text-slate-900">Konfirmasi Hapus</h3>
                <button type="button" class="modal-close text-slate-500 hover:text-slate-900">✕</button>
            </div>
            <form id="deleteForm" method="POST" class="px-8 py-6 space-y-4">
                @csrf
                @method('DELETE')
                <p class="text-sm text-slate-600">Apakah Anda yakin ingin menghapus resepsionis <span id="deleteNameLabel" class="font-bold text-forest-900"></span>? Tindakan ini permanen.</p>
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" class="modal-close px-6 py-3 rounded-xl bg-slate-100 text-slate-700 font-bold hover:bg-slate-200 transition">Batal</button>
                    <button type="submit" class="px-6 py-3 rounded-xl bg-red-600 text-white font-bold hover:bg-red-700 transition">Ya, Hapus</button>
                </div>
            </form>
        </div>
    </div>

    <script src="{{ asset('js/admin/crudresepsionis.js') }}"></script>
</body>

</html>
