...
<body class="bg-gray-50 min-h-screen flex">
    ...
    <main class="flex-1 overflow-y-auto px-10 py-10">
        ...
        <div class="flex justify-between items-end mb-10 fade-up">
            <div>
                <h1 class="font-display text-4xl font-bold text-forest-900">
                    Data Tamu
                </h1>

                <p class="text-forest-500 mt-2 text-sm uppercase tracking-widest font-semibold">
                    Manajemen Data Tamu & Informasi
                </p>
            </div>
                ...
                <table class="w-full text-left whitespace-nowrap">
                    ...
                    <thead>
                        <tr class="bg-forest-800 text-white text-[11px] uppercase tracking-[0.2em]">
                            ...
                            <th class="px-6 py-6 font-semibold">
                                Alamat Email
                            </th>

                            <th class="px-6 py-6 font-semibold text-center">
                                No WA
                            </th>
                            ...
                            <th class="px-8 py-6 font-semibold text-center border-l border-forest-700">
                                Aksi
                            </th>
                        </tr>
                    </thead>
                    ...
                    <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-forest-50/50 transition-colors group">
                                ...
                                <td class="px-8 py-6">
                                    @if($item->photo)
                                        <img src="{{ asset('storage/' . $item->photo) }}"
                                            class="w-14 h-14 rounded-full object-cover border-2 border-forest-200">
                                    @else
                                        <div
                                            class="w-14 h-14 rounded-full bg-forest-700 text-white flex items-center justify-center font-bold text-lg">
                                ...
                                <td class="px-8 py-6">
                                    <div class="flex items-center gap-3">
                                        <span class="font-bold text-forest-900">
                                            {{ $item->name }}
                                        </span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-6 text-center text-sm text-black">
                                    {{ $item->whatsapp ?? '-' }}
                                </td>
                                ...
                                <td class="px-6 py-6 text-center text-sm text-black">
                                    TMU-{{ str_pad($item->id, 3, '0', STR_PAD_LEFT) }}
                                </td>

                                <td class="px-8 py-6 border-l border-forest-600/40">
                                    <div class="flex justify-center gap-3">
                                        <button data-id="{{ $item->id }}" data-name="{{ $item->name }}"
                                            data-email="{{ $item->email }}" data-whatsapp="{{ $item->whatsapp }}"
                                            data-birthday="{{ $item->birthday }}" data-username="{{ $item->username }}"
                                            class="btn-edit px-5 py-3 bg-amber-100 text-amber-600 rounded-md hover:bg-amber-200 transition-colors text-[10px] font-bold uppercase tracking-wider">
                                            Edit
                                        </button>

                                        <button data-id="{{ $item->id }}" data-name="{{ $item->name }}"
                                            class="btn-delete px-5 py-3 bg-red-100 text-red-600 rounded-md hover:bg-red-200 transition-colors text-[10px] font-bold uppercase tracking-wider">
                                            Hapus
                                        </button>
                                    </div>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-8 py-10 text-center text-sm text-gray-500 font-semibold">
                                    Belum ada data tamu.
                                </td>
                            </tr>
                        @endforelse
                ...
                <div class="space-y-4 overflow-y-auto max-h-[60vh] pr-1">
                    ...
                    <div>
                        ...
                        <input name="email" type="email" required
                            class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm"
                            placeholder="Contoh: guest@gmail.com">
                    </div>
    ...
    <div id="editModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 px-4 py-8">
        <div class="w-full max-w-lg rounded-3xl bg-white shadow-2xl ring-1 ring-black/5 overflow-hidden">
            ...
            <form id="editForm" method="POST" enctype="multipart/form-data" class="space-y-6 px-8 py-6">
                @csrf
                @method('PUT')

                <div class="space-y-4 overflow-y-auto max-h-[60vh] pr-1">
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Ganti Foto Profil</label>
                        <input name="photo" type="file" accept="image/*"
                            class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm">
                    </div>
                    ...
                    <div>
                        <label class="text-sm font-semibold text-slate-700">No WhatsApp</label>
                        <input id="editWhatsapp" name="whatsapp" type="text" required
                            class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm">
                    </div>

                    <div>
                        <label class="text-sm font-semibold text-slate-700">Tanggal Lahir</label>
                        <input id="editTanggalLahir" name="birthday" type="date" required
                            class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm">
                    </div>
        ...
        <div class="w-full max-w-md rounded-3xl bg-white shadow-2xl ring-1 ring-black/5 overflow-hidden">
            ...
            <form id="deleteForm" method="POST" class="px-8 py-6 space-y-4">
                ...
                <p class="text-sm text-slate-600">
                    ...
                    <span id="deleteNameLabel" class="font-bold text-forest-900"></span>?
                </p>
