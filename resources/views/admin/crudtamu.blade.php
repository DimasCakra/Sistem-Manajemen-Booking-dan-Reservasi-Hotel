...
<body class="bg-gray-50 min-h-screen flex">
                        ...
                        @forelse ($tamus as $item)
                            <tr class="hover:bg-forest-50/50 transition-colors group">
                                ...
                                <td class="px-6 py-6 text-center">
                                    <span class="bg-blue-100 text-blue-700 px-5 py-2 rounded-lg text-xs font-semibold">@ {{ $item->username ?? '-' }}</span>
                                </td>
                                ...
                                <td class="px-8 py-6 border-l border-forest-600/40">
                                    <div class="flex justify-center gap-3">
                                        <button data-id="{{ $item->id }}" data-name="{{ $item->name }}" data-email="{{ $item->email }}" data-whatsapp="{{ $item->whatsapp }}" data-username="{{ $item->username }}" class="btn-edit px-5 py-3 bg-amber-100 text-amber-600 rounded-md hover:bg-amber-200 transition-colors text-[10px] font-bold uppercase tracking-wider">Edit</button>
                                        <button data-id="{{ $item->id }}" data-name="{{ $item->name }}" class="btn-delete px-5 py-3 bg-red-100 text-red-600 rounded-md hover:bg-red-200 transition-colors text-[10px] font-bold uppercase tracking-wider">Hapus</button>
                                    </div>
                                </td>
                            </tr>
                        @empty
    ...
    <div id="createModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 px-4 py-8">
        <div class="w-full max-w-lg rounded-3xl bg-white shadow-2xl ring-1 ring-black/5 overflow-hidden">
            ...
            <form action="{{ route('admin.tamu.store') }}" method="POST" class="space-y-6 px-8 py-6">
                @csrf
                <div class="space-y-4">
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Nama Lengkap</label>
                        <input name="name" type="text" required class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none" placeholder="Masukkan nama lengkap" />
                    </div>
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Alamat Email</label>
                        <input name="email" type="email" required class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none" placeholder="Contoh: guest@gmail.com" />
                    </div>
                    <div>
                        <label class="text-sm font-semibold text-slate-700">No WhatsApp</label>
                        <input name="whatsapp" type="text" required class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none" placeholder="Contoh: 08123456789" />
                    </div>
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Username (Opsional)</label>
                        <input name="username" type="text" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none" placeholder="Contoh: guest_123" />
                    </div>
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Password</label>
                        <input name="password" type="password" required minlength="8" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none" placeholder="Minimal 8 karakter" />
                    </div>
                </div>
    ...
    <div id="editModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 px-4 py-8">
        <div class="w-full max-w-lg rounded-3xl bg-white shadow-2xl ring-1 ring-black/5 overflow-hidden">
            <div class="flex items-center justify-between px-8 py-6 border-b border-gray-200">
                <div>
                    <h2 class="text-2xl font-bold text-slate-900">Edit Data Tamu</h2>
                    <p class="text-sm text-slate-500 mt-1">Perbarui informasi akun tamu.</p>
                </div>
                <button type="button" class="modal-close text-slate-500 hover:text-slate-900" aria-label="Tutup">✕</button>
            </div>
            <form id="editForm" method="POST" class="space-y-6 px-8 py-6">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Nama Lengkap</label>
                        <input id="editName" name="name" type="text" required class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none" placeholder="Masukkan nama lengkap" />
                    </div>
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Alamat Email</label>
                        <input id="editEmail" name="email" type="email" required class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none" placeholder="Contoh: guest@gmail.com" />
                    </div>
                    <div>
                        <label class="text-sm font-semibold text-slate-700">No WhatsApp</label>
                        <input id="editWhatsapp" name="whatsapp" type="text" required class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none" placeholder="Contoh: 08123456789" />
                    </div>
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Username (Opsional)</label>
                        <input id="editUsername" name="username" type="text" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none" placeholder="Contoh: guest_123" />
                    </div>
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Password Baru</label>
                        <input name="password" type="password" minlength="8" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:border-forest-500 focus:ring-4 focus:ring-forest-100 outline-none" placeholder="Kosongkan jika tidak ingin mengubah" />
                    </div>
                </div>
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" class="modal-close px-6 py-3 rounded-xl bg-slate-100 text-slate-700 font-bold hover:bg-slate-200 transition">Batal</button>
                    <button type="submit" class="px-6 py-3 rounded-xl bg-forest-700 text-white font-bold hover:bg-forest-800 transition">Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 px-4 py-8">
        <div class="w-full max-w-md rounded-3xl bg-white shadow-2xl ring-1 ring-black/5 overflow-hidden">
            ...
            <form id="deleteForm" method="POST" class="px-8 py-6 space-y-4">
                @csrf
                @method('DELETE')
                <p class="text-sm text-slate-600">Apakah Anda yakin ingin menghapus data tamu <span id="deleteNameLabel" class="font-bold text-forest-900"></span>? Tindakan ini tidak dapat dibatalkan.</p>
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" class="modal-close px-6 py-3 rounded-xl bg-slate-100 text-slate-700 font-bold hover:bg-slate-200 transition">Batal</button>
