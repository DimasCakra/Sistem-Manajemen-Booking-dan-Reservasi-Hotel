...
<body class="bg-gray-50 min-h-screen flex">
            ...
            <div class="flex gap-3 w-full md:w-auto">
                <select name="type" id="filterType" class="w-full md:w-44 px-4 py-3 bg-white border border-forest-100 rounded-xl text-sm focus:border-forest-500 outline-none text-slate-700 font-medium cursor-pointer">
                    ...
                    @foreach($listTipe as $t)
                        <option value="{{ $t->id_tipe_kamar }}" {{ (isset($type) && $type == $t->id_tipe_kamar) ? 'selected' : '' }}>{{ $t->nama_tipe }}</option>
                    @endforeach
                </select>
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-gray-100" id="roomTableBody">
                    @forelse($kamars as $kamar)
                        @php
                            $fotos = json_decode($kamar->foto_kamar, true) ?? [];
                        ...
                        <tr class="hover:bg-forest-50/50 transition-colors group"
                            ...
                            data-room-type-name="{{ $kamar->nama_tipe }}"
                            data-price="{{ $kamar->harga_per_malam }}"
                            data-room-status="{{ $kamar->status_kamar }}"
                            data-room-description="{{ $kamar->deskripsi }}"
                            data-room-images="{{ json_encode($fotoUrls) }}">
                            <td class="px-8 py-6 font-bold text-black">{{ $kamar->no_kamar }}</td>
                            <td class="px-6 py-6 text-center text-sm text-black">{{ $kamar->nama_tipe }}</td>
                            ...
                            <td class="px-8 py-6 text-xs text-gray-500 max-w-[200px] truncate">{{ $kamar->deskripsi }}</td>
                            <td class="px-8 py-6 border-l border-forest-600/40">
                                <div class="flex justify-center gap-4">
                                    <button type="button" class="btn-view text-blue-600 hover:text-blue-800" title="Detail Kamar Templates">
                                    ...
                                    <form action="{{ route('admin.kamar.destroy', $kamar->no_kamar) }}" method="POST" class="inline">
                                        @csrf @method('DELETE')
                                        <button type="submit" onclick="return confirm('Yakin ingin menghapus unit kamar ini?')" class="text-red-500 hover:text-red-700" title="Hapus Kamar">
    ...
    <div id="createModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 px-4 py-6">
        <div class="w-full max-w-md rounded-3xl bg-white shadow-2xl flex flex-col overflow-hidden">
            ...
            <form action="{{ route('admin.kamar.store') }}" method="POST" class="px-8 py-5 space-y-4">
                @csrf
                <div>
                    <label class="text-sm font-semibold text-slate-700">No Kamar</label>
                    <input name="no_kamar" type="text" required class="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-forest-500" placeholder="Masukkan nomor kamar" />
                </div>
                <div>
                    <label class="text-sm font-semibold text-slate-700">Tipe Kamar</label>
                    <select name="id_tipe_kamar" required class="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none cursor-pointer focus:border-forest-500">
                        <option value="">Pilih Tipe Kamar</option>
                        @foreach($listTipe as $tipe)
                            <option value="{{ $tipe->id_tipe_kamar }}">{{ $tipe->nama_tipe }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="text-sm font-semibold text-slate-700">Status</label>
                    <select name="status_kamar" required class="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-forest-500">
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit" class="px-6 py-2.5 rounded-xl bg-forest-700 text-white font-bold text-sm">Simpan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="detailModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 px-4 py-6">
        <div class="w-full max-w-2xl rounded-3xl bg-white shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
            <div class="flex items-center justify-between px-8 py-5 border-b border-gray-100 flex-shrink-0">
                <div>
                    ...
                    <p id="detailModalSubtitle" class="text-sm text-slate-500 mt-0.5">Lihat spesifikasi template master atau ubah unit.</p>
                </div>
                <button type="button" class="modal-close text-slate-400 hover:text-slate-600">✕</button>
            </div>

            <form id="detailModalForm" method="POST" action="" class="flex-1 overflow-y-auto px-8 py-5 space-y-5">
                @csrf @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-5">
                    ...
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Tipe Kamar (Inherited)</label>
                    ...
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Harga (Template)</label>
                        <input id="detailRoomPrice" type="text" class="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm bg-gray-100 text-gray-500 outline-none" readonly />
                    </div>
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Status Unit</label>
                    ...
                    <div class="col-span-2">
                        <label class="text-sm font-semibold text-slate-700">Deskripsi Ringkas Tipe</label>
                        <textarea id="detailRoomDescription" class="mt-1.5 w-full h-32 rounded-2xl border border-slate-200 px-4 py-2.5 text-sm bg-gray-100 text-gray-500 outline-none resize-none" readonly></textarea>
                    </div>
                </div>

                <div>
                    <label class="text-sm font-semibold text-slate-700">Album Foto Tipe Kamar</label>
                    <div id="detailRoomImageContainer" class="mt-2 grid grid-cols-2 sm:grid-cols-3 gap-3 p-2 bg-slate-50 border border-slate-100 rounded-2xl min-h-[100px] items-center justify-center"></div>
                </div>
