                ...
                <tbody class="divide-y divide-gray-100" id="roomTableBody">
                    @forelse($kamars as $kamar)
                        @php
                            ...
                            $fotoUrls = array_map(function($foto) {
        ...
        <div class="w-full max-w-2xl rounded-3xl bg-white shadow-2xl ring-1 ring-black/5 flex flex-col max-h-[90vh] overflow-hidden">
            <div class="flex items-center justify-between px-8 py-5 border-b border-gray-100 flex-shrink-0">
                <div>
                    ...
                    <p class="text-sm text-slate-500 mt-0.5">Maksimal upload 5 foto sekaligus.</p>
                </div>
                <button type="button" class="modal-close text-slate-400 hover:text-slate-600 transition">✕</button>
            </div>
            <form action="{{ route('admin.kamar.store') }}" method="POST" enctype="multipart/form-data" class="flex-1 overflow-y-auto px-8 py-5 space-y-5">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <select name="tipe_kamar" required class="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm text-slate-900 outline-none cursor-pointer focus:border-forest-500">
                    ...
                    <div>
                        ...
                        <input name="harga_per_malam" type="number" min="0" required class="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm text-slate-900 outline-none focus:border-forest-500" placeholder="Masukkan harga" />
                    </div>
                </div>
                <div>
                    <label class="text-sm font-semibold text-slate-700">Deskripsi</label>
                    <textarea name="deskripsi" rows="3" class="mt-1.5 w-full rounded-2xl border border-slate-200 px-4 py-2.5 text-sm text-slate-900 outline-none resize-none focus:border-forest-500" placeholder="Tuliskan detail kamar..."></textarea>
                </div>
                <div>
                    <label class="text-sm font-semibold text-slate-700">Upload Foto Kamar (Maks 5 Foto)</label>
                    <input name="foto_kamar[]" type="file" accept="image/*" multiple class="mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm text-slate-900 outline-none file:mr-4 file:py-1 file:px-4 file:rounded-md file:border-0 file:bg-forest-50 file:text-forest-700 hover:file:bg-forest-100 cursor-pointer" />
                </div>
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-gray-100 sticky bottom-0 bg-white z-10 pb-1">
                    ...
                    <button type="submit" class="px-6 py-2.5 rounded-xl bg-forest-700 text-white font-bold text-sm">Create</button>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="w-full max-w-2xl rounded-3xl bg-white shadow-2xl ring-1 ring-black/5 flex flex-col max-h-[90vh] overflow-hidden">
            <div class="flex items-center justify-between px-8 py-5 border-b border-gray-200 flex-shrink-0">
                <div>
                    ...
                    <p id="detailModalSubtitle" class="text-sm text-slate-500 mt-0.5">Lihat detail atau edit data kamar.</p>
                </div>
                <button type="button" class="modal-close text-slate-400 hover:text-slate-600 transition">✕</button>
            </div>
            <form id="detailModalForm" method="POST" action="" enctype="multipart/form-data" class="flex-1 overflow-y-auto px-8 py-5 space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Tipe Kamar</label>
                        <select id="detailRoomType" name="tipe_kamar" class="modal-field mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm text-slate-900 outline-none cursor-pointer" disabled>
                            <option value="Kamar Deluxe">Kamar Deluxe</option>
                            <option value="Kamar Superior">Kamar Superior</option>
                            <option value="Kamar Suite">Kamar Suite</option>
                        </select>
                    ...
                    <div>
                        <label class="text-sm font-semibold text-slate-700">Harga</label>
                        <input id="detailRoomPrice" name="harga_per_malam" type="number" min="0" class="modal-field mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm text-slate-900 outline-none" readonly />
                    </div>
                    <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    ...
                    <div class="md:col-span-2">
                        <label class="text-sm font-semibold text-slate-700">Deskripsi</label>
                        <textarea id="detailRoomDescription" name="deskripsi" rows="3" class="modal-field mt-1.5 w-full rounded-2xl border border-slate-200 px-4 py-2.5 text-sm text-slate-900 outline-none resize-none" readonly></textarea>
                    </div>
                </div>
                <div>
                    <label class="text-sm font-semibold text-slate-700">Ganti Semua Foto Kamar (Maks 5 Foto - Opsional)</label>
                    <input id="detailRoomImageInput" name="foto_kamar[]" type="file" accept="image/*" multiple class="modal-field mt-1.5 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm text-slate-900 outline-none file:mr-4 file:py-1 file:px-4 file:rounded-md file:border-0 file:bg-forest-50 file:text-forest-700 hover:file:bg-forest-100 cursor-pointer" disabled />
                </div>
                <div>
